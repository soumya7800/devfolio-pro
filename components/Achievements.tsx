...
const miniStats = [['500+', 'Problems Solved'], ['Top 15%', 'Global Rank'], ['50 Day', 'Streak']];

export const Achievements: React.FC = () => {
  return (
    <section id="achievements" className="py-24 relative overflow-hidden border-b border-borderSubtle scroll-mt-24">
      {/* Multi-layer ambient glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px] h-80 bg-accent/8 rounded-full blur-[120px] pointer-events-none animate-breathe" />
      <div className="absolute bottom-0 right-1/4 w-64 h-64 bg-warning/6 rounded-full blur-[100px] pointer-events-none animate-breathe" style={{ animationDelay: '4s' }} />
      ...
      <div className="relative z-10 w-full">
        ...
        >
          <TiltCard intensity={6} className="group relative glass-focus glass-highlight glass-noise rounded-2xl overflow-hidden shimmer-card w-full"
            ...
            <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-accent/4 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-2xl" />

            {/* Ambient inner glow spots */}
            <div className="absolute top-0 right-0 w-64 h-64 bg-warning/6 rounded-full blur-[80px] pointer-events-none" />
            <div className="absolute bottom-0 left-0 w-48 h-48 bg-accent/6 rounded-full blur-[60px] pointer-events-none" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-5">
              ...
              <div className="md:col-span-3 p-8 md:p-12 flex flex-col gap-6 relative z-10">
                ...
                <div className="flex flex-col">
                  <motion.span
                    ...
                    className="font-sans font-black leading-none text-gradient-accent glow-number"
                    style={{ fontSize: 'clamp(5rem, 12vw, 8rem)', color: '#3DDBFF' }}
                  >
                    50
                  </motion.span>
                ...
                {/* Title and desc */}
                <div>
                  <h3 className="font-sans font-black text-2xl text-white mb-3 group-hover:text-gradient-accent transition-all">
                ...
                <div className="flex gap-5">
                  {miniStats.map(([val, label], i) => (
                    ...
                    >
                      <span className="font-sans font-black text-lg text-accent glow-number">{val}</span>
                      <span className="font-mono text-[9px] text-muted uppercase tracking-widest">{label}</span>
                    </motion.div>
                  ))}
                </div>

                {/* CTA — Glass/Solid Hybrid + Magnetic */}
                <MagneticButton
                  href="https://leetcode.com/medal/?showImg=0&id=9517581&isLevel=false"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 self-start btn-glass-hybrid px-5 py-2.5 rounded-xl font-bold text-sm"
                >
                  <Trophy size={15} className="relative z-10" />
              ...
              <div className="md:col-span-2 relative flex items-center justify-center p-8 border-t md:border-t-0 md:border-l border-borderSubtle overflow-hidden bg-surfaceDeep/40 min-h-[280px]">
                ...
                <div
                  ...
                  style={{ clipPath: 'polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)', boxShadow: '0 0 40px rgba(61,219,255,0.15)' }}
                >
                  <img
